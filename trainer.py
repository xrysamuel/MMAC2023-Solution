...
class Trainer:
    ...
    def _criterion(self, outputs, labels):
        ...
        if isinstance(outputs, tuple) and len(outputs) == 2:
            ...
            raise TypeError(f"Unsupported outputs type: {type(outputs)}. Expected torch.Tensor or a tuple containing logits.")
    ...
    def greedy_soup(self):
        ...
        current_best_accuracy = -1.0 # Initialize with a value lower than any possible accuracy
        ...
        for path in model_paths:
            ...
            try:
                ...
                logger.error(f"Failed to load state_dict from {path}: {e}. Skipping this model.")
        ...
        for _ in range(len(candidate_state_dicts)): 
            ...
            for j in range(len(candidate_state_dicts)):
                ...
                for key in candidate_state_dicts[j].keys(): 
                    if soup_model_state_dict is None: # This happens for the very first model selected
                        temp_soup_model_state_dict[key] = candidate_state_dicts[j][key]
                    else:
                        temp_soup_model_state_dict[key] = (soup_model_state_dict[key] * selected_models_count + candidate_state_dicts[j][key]) / num_models_in_proposed_soup
                ...
                with temp_eval(temp_model_for_eval) as eval_model:
                    _, temp_accuracy = self._evaluation(eval_loader, eval_model, criterion)

                logger.info(f"Proposing to add '{candidate_names[j]}'. Proposed Soup Accuracy: {temp_accuracy:.4f}")
            ...
            if best_ingredient_idx_this_iter != -1 and best_temp_accuracy_this_iter > current_best_accuracy:
                ...
                if soup_model_state_dict is None: # Should only happen for the first selected model
                    soup_model_state_dict = candidate_state_dicts[best_ingredient_idx_this_iter]
                else:
                    # Recompute the average for the current soup with the newly added model
                    for key in soup_model_state_dict.keys():
                        soup_model_state_dict[key] = (soup_model_state_dict[key] * (selected_models_count - 1) + candidate_state_dicts[best_ingredient_idx_this_iter][key]) / selected_models_count

                current_best_accuracy = best_temp_accuracy_this_iter
                logger.info(f"GREEDY SOUP PROGRESS: Added '{candidate_names[best_ingredient_idx_this_iter]}'. New Best Soup Accuracy: {current_best_accuracy:.4f}")
            else:
                logger.info(f"No further improvement by adding remaining models or no valid models to add. Greedy Soup finished.")
        ...
        if soup_model_state_dict is not None:
            ...
            final_soup_model = self._get_model_no_logging() # Initialize a fresh model for the soup
            ...
            logger.info(f"Final Greedy Soup Accuracy achieved: {current_best_accuracy:.4f}")
        else:
            logger.warning("Greedy Soup did not result in any combined model (perhaps no models improved accuracy).")